...
function ReleasesPage({releases,session,onChanged,onBase,onEngine}:any) {
  ...
  const filtered=releases.filter((r:any)=>{if(type!=="all"&&String(r.release_type)!==type)return false;if(state!=="all"){const s=(lifecycleFor(r)?.archived||r.archived_at)?"archived":String(r.status||"draft");if(s!==state)return false;}if(channel!=="all"&&String(r.channel)!==channel)return false;const q=query.trim().toLowerCase();if(q&&!String(r.version||"").toLowerCase().includes(q)&&!String(r.source_sha||"").toLowerCase().includes(q)&&!String(r.product_name||"orbitfs").toLowerCase().includes(q))return false;return true;});
  const act=async(row:any,action:"approve"|"reject"|"rollback"|"withdraw"|"archive"|"restore")=>{setBusy(row.id+":"+action);setError("");setMessage("");try{let reason:string|undefined;if(action==="rollback"||action==="archive"){reason=prompt(action==="rollback"?"Reason for rollback:":"Reason for archiving this release:","")||undefined;if(!reason?.trim()){setBusy("");return}}await controlRelease({data:{token:session.token,releaseId:row.id,action,reason}});setMessage("Release action completed: "+action+".");await onChanged?.()}catch(x:any){setError(x.message||"Release action failed.")}finally{setBusy("")}};
...
function ChannelsPage({channels,data,session}:any) {
  const [state,setState]=useState<any>({channels:[],requests:[],access:[],warnings:[]});
  const [loading,setLoading]=useState(true);
  const [error,setError]=useState("");
  const [notice,setNotice]=useState("");
  ...
  return <section className="space-y-4">
    ...
    {error&&<Alert tone="error" onClose={()=>setError("")}>{error}</Alert>}{notice&&<Alert tone="success" onClose={()=>setNotice("")}>{notice}</Alert>}{state.warnings?.length>0&&<Alert tone="warning">{state.warnings.join(" ")}</Alert>}
